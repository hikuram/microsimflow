...
def create_montage_exp9(df, seed, out_filename, scale_factor=2.0):
    ...
    title_text = f"Exp9: Fibers Structure (Seed {int(seed)})"
...
def plot_exp9():
    csv_file = "exp9_compare_fibers_results.csv"
    ...
    print("\n--- Generating Exp9 Montages ---")
    seeds = sorted(df['Seed'].unique())
    for seed in seeds:
        create_montage_exp9(df, seed=seed, out_filename=f"exp9_fibers_montage_seed{int(seed)}.png")

    print("\n--- Plotting Exp9 Conductivity ---")
    ...
    plt.yscale('log')
    plt.title('Exp9: Conductivity (Txx) vs Volume Fraction (VF) for Fibers')
    plt.xlabel('Volume Fraction (VF)')
    plt.ylabel('Conductivity (Txx) [Log Scale]')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.grid(True, which="both", ls="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig('exp9_fibers_conductivity_plot.png', dpi=300)
    print("Saved plot: exp9_fibers_conductivity_plot.png")
