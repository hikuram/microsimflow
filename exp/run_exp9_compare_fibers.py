...
def parse_args():
    parser = argparse.ArgumentParser(description="Exp9: solver comparison with isotropic random fibers and 3 interface profiles")
    parser.add_argument("--solver", default="both", choices=["chfem", "puma", "both", "skip"])
    parser.add_argument("--size", type=int, default=160)
    parser.add_argument("--seeds", nargs="+", type=int, default=[0, 1, 2])
    parser.add_argument("--vfs", nargs="+", type=float, default=[0.03, 0.06, 0.09, 0.12, 0.15])
...
def run():
    args = parse_args()
    csv_log = "exp9_compare_fibers_results.csv"
    out_dir = get_next_result_dir("result_exp9_")
    os.makedirs(out_dir)
    ...
    print(f"Starting Exp9. Logging to '{csv_log}'")
    ...
    for profile_name, props in interface_profiles.items():
        for vf in args.vfs:
            for seed in args.seeds:
                ...
                basename = os.path.join(out_dir, f"exp9_{profile_name}_vf{vf:.2f}_seed{seed}")
